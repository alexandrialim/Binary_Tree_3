...
public class BinaryTree {
	...
	private int subtreeMinRecord(Node topNode){
		...
		public void removeNode(int key){
			root = subtreeDelete(root, key);
		}
		private Node subtreeDelete(Node topNode, int key){
			if (topNode == null) {
				return null;
			} 				
			if (key <  topNode.record) {
				topNode.left = subtreeDelete(topNode.left, key);
			}
			else if(key > topNode.record){
				topNode.right = subtreeDelete(topNode.right, key);
			}
			else{
				if(topNode.left != null && topNode.right != null){
					int i = subtreeMinRecord(topNode.right);
					topNode.record = i;
					topNode.right = subtreeDelete(topNode.right, i);
